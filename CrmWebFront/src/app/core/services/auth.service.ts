...
import { SetIdentifiantUser } from '@core/store/actions/user.action';
import { IAppState } from '@core/store/state/app.state';
import { Store } from '@ngrx/store';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';
import { IToken, JwtDto, Token, tokenAccessKey, UserDto } from 'src/app/shared/models/token';
...
export class AuthService {
  ...
  changePasswordConnected(login: string, password: string, passwordActuel: string): Observable<boolean> {
    const token = this.cookieService.get(tokenAccessKey) as IToken;
    const headers = new HttpHeaders({
      authorization: token.accessToken.toString(),
      // eslint-disable-next-line @typescript-eslint/naming-convention
      'Content-Encoding': btoa(login + ':' + password + ':' + passwordActuel + ':' + token.accessToken.toString()),
    });
    return this.http.put<boolean>('/changepasswordconnected', {}, { headers }).pipe(map((result) => result));
  }
  ...
  public decodeToken(token: string): UserDto {
    ...
    this.store.dispatch(new SetIdentifiantUser(user.sub));
    return user;
  }
}
