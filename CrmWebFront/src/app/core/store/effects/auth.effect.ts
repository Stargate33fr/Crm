...
export class AuthEffects {
  ...
  changePasswordConnected$ = createEffect(() =>
    this.actions$.pipe(
      ofType<ChangePassword>(AuthActionType.changePasswordConnected),
      map((action) => action.payload),
      switchMap((payload) =>
        this.authService.changePasswordConnected(payload.email, payload.password, payload.passwordActuel).pipe(
          map((result: boolean) => new ChangePasswordSucess({ success: result })),
          catchError(() => of(new ChangePasswordFailure(payload.email))),
        ),
      ),
    ),
  );
  ...
  logInSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap((action) => {
          this.cookieService.set(tokenAccessKey, action.payload, action.payload.expireIn);
