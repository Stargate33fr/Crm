...
import { ParametreChange } from '@models/parametrages';
...
export class MoleculesInputNumberComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.valueAutre = this.valeurReel ? this.formatValeurForce(this.valeurReel.toString()) : '';
  }
  ...
  updateValue(value: string): void {
    ...
    this.inputElement.nativeElement.value = this.valueAutre;
    const changementValeur = new ParametreChange();
    changementValeur.id = +this.id;
    changementValeur.valeur = value;

    this.changeValeur.emit(changementValeur);
  }
